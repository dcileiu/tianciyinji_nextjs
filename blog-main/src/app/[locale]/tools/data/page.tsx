...
export default async function DataToolsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      <JsonLd data={buildCollectionPageJsonLd({ title: pageTitle(text.title), description: text.description, path: '/tools/data', locale })} />
      <ToolsClientPage section="public-data-tools" />
    </>
  );
}

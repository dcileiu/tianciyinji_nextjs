...
export default async function SeoToolsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      <JsonLd data={buildCollectionPageJsonLd({ title: pageTitle(text.title), description: text.description, path: '/tools/seo', locale })} />
      <ToolsClientPage section="seo-geo-tools" />
    </>
  );
}

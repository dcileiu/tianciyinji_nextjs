...
import { getDictionary, localizedHref, normalizeLocale } from '@/lib/i18n';
...
export default async function ResourceDetailPage({ params }: { params: Promise<{ slug: string; locale: string }> }) {
  const { slug, locale: localeParam } = await params;
  const locale = normalizeLocale(localeParam);

...
export function useMusicPlayer() {
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const newTracks = data.tracks.filter((newTrack) => !store.playlist.some((track) => track.id === newTrack.id));
  ...
  // 上一首
  const playPrevious = () => {
    store.setCurrentTrackIndex(store.currentTrackIndex === 0 ? store.playlist.length - 1 : store.currentTrackIndex - 1);
  };

...
import { getLyric, getMusicUrl, getPlaylistSongs } from '@/lib/music';
...
export function MusicPlayerProvider({ children }: MusicPlayerProviderProps) {
  ...
  const parseLyrics = useCallback(
    (lyrics: { original?: string; translated?: string }) => {
      ...
      const parsed: LyricLine[] = original
        .split('\n')
        .map((line) => {
    ...
    const run = async () => {
      ...
      try {
        const [urlData, lyricData] = await Promise.all([
          getMusicUrl(Number(currentSongId)),
          getLyric(Number(currentSongId)),
        ]);

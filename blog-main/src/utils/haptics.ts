...
const matchMediaFn = (q: string) =>
  hasWindow && typeof window.matchMedia === 'function' ? window.matchMedia(q) : null;
...
const makeCapacitorAdapter = (): HapticsAdapter | null => {
  ...
  const notification = async (type: 'success' | 'warning') => {
    const candidates = [{ type: 'success' }, { type: 'SUCCESS' }, { type: 'warning' }, { type: 'WARNING' }];
  ...
  const selectionStart = async () => {
    await safeCall(() => H.selectionStart?.());
  };
  const selectionEnd = async () => {
    await safeCall(() => H.selectionEnd?.());
  };
...
const makeTapticAdapter = (): HapticsAdapter | null => {
  ...
  const notification = (type: 'success' | 'warning') => {
    T.notification({ type });
  };
  const selection = () => {
    T.selection();

...
function shouldExclude(path: string): boolean {
  return (
    path.startsWith('.obsidian/') || path.startsWith('Temp Book/') || path.startsWith('.') || !path.endsWith('.md')
  );
}
...
export async function processIncrementalUpdate(commit: WebhookCommit): Promise<ObsidianIndex> {
  ...
  const toUnpublish = buildResults
    .filter(Boolean)
    .filter((r: any) => r?.unpublished)
    .map((r: any) => r.slug);
  const encryptedSlugs = buildResults
    .filter(Boolean)
    .filter((r: any) => r?.encrypted)
    .map((r: any) => r.slug);

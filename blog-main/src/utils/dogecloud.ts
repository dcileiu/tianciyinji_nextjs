...
function generateAuthToken(requestUri: string, body = ''): string {
  ...
  const sign = crypto.createHmac('sha1', OSS_SECRET_KEY).update(signStr, 'utf-8').digest('hex');
...
export async function uploadToDogeCloud(buffer: Buffer, fileName: string): Promise<string> {
  try {
    ...
    const response = await axios.put(`${API_HOST}${requestUri}`, buffer, {
      headers: {

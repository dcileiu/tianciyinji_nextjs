...
// 带重试的请求函数
async function requestWithRetry<T>(fn: () => Promise<T>, retries = 3, delayMs = 1000): Promise<T> {
  try {
    return await fn();
  } catch (error) {
...
// 并发控制函数
async function asyncPool<T, U>(poolLimit: number, array: T[], iteratorFn: (item: T) => Promise<U>): Promise<U[]> {
...
async function getSongUrl(id: string): Promise<string | null> {
  try {
    ...
    const res = await requestWithRetry(() => axios.get(`${BASE_URL}/song/url/v1?id=${id}&level=exhigh`));
...
async function getSongDetail(ids: string[]): Promise<SongDetail[]> {
  try {
    const res = await requestWithRetry(() => axios.get(`${BASE_URL}/song/detail?ids=${ids.join(',')}`));
...
export async function getLyric(id: string): Promise<string> {
  try {
    const res = await requestWithRetry(() => axios.get(`${BASE_URL}/lyric?id=${id}`));

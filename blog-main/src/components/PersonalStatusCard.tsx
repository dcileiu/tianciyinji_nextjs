...
const getActivityIcon = (activityText: string, processName: string) => {
  ...
  if (text.includes('game') || text.includes('玩') || text.includes('游') || text.includes('the finals'))
          ...
          <div className="p-5 sm:p-6">
            <div className="flex items-center justify-between mb-3 sm:mb-4">
              <div className="flex items-center space-x-2">
                <span className={status.alive.color}>{aliveIcon}</span>
                <p className={`text-xs sm:text-sm font-medium ${status.alive.color}`}>{status.alive.text}</p>
              </div>
            </div>
            ...
            >
              ...
              <ArrowUpRight
                size={14}
                className="ml-1 opacity-70 group-hover/link:opacity-100 transition-opacity duration-200"
              />
            </a>
          </div>
        </div>

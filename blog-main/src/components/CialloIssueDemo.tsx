...
const DEMO_MESSAGES: Message[] = [
  {
    ...
    role: "user",
    content:
      "导出为HTML时，是否可以将该条消息的 `message_id` 作为div的id属性？方便使用JS快速滚动定位到某条消息的位置",
  },
  {
    id: 2,
    author: "Ciallo",
    ...
    role: "ciallo",
    content:
      "看了一下，这个功能确实很实用！通过 `message_id` 快速定位消息对于长聊天记录特别有帮助～",
  },
  {
    id: 3,
    author: "Ciallo",
          ...
          setTimeout(() => {
            ...
            requestAnimationFrame(() => {
              requestAnimationFrame(() => {
                ...
                // 找到 footer 元素并获取其高度
                const footerElement = content.querySelector(
                  ".footer-hero",
                ) as HTMLElement;
                ...
                const targetTranslate = -(
                  footerTop -
                  (containerHeight - footerHeight) / 2
                );
                ...
                const animateTransform = (currentTime: number) => {
                  ...
                  const newTranslate =
                    startTranslate + distance * easedProgress;

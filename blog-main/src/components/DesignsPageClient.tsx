...
export default function DesignsPageClient() {
  ...
  const labels =
    locale === 'en'
      ? {
          ...
          searchPlaceholder: 'Search type, style, tag...',
          noResults: 'No related designs found',
          searchEmpty: 'Enter keywords to search designs',
        }
      : {
          all: '全部',
          search: '搜索',
          random: '随机',
          flipBack: '点击翻回',
          searchPlaceholder: '搜索类型、风格、标签...',
          noResults: '没有找到相关设计',
          searchEmpty: '输入关键词搜索设计',
        };
  ...
  // 筛选后的设计
  const designs = filter ? allDesigns.filter((d) => d.type === filter || d.styles.includes(filter)) : allDesigns;

  // 获取所有类型
  const types = Array.from(new Set(allDesigns.map((d) => d.type))).filter(Boolean);
              ...
              >
                ...
                >
                  <div className="bg-neutral-900 rounded-lg p-6 md:p-12 max-w-lg w-full h-full flex flex-col justify-center">
                    ...
                    <div className="text-white/40 text-xs md:text-sm mb-4 md:mb-6">
                      {d.mood} · @{d.user_name}
                    </div>
                    ...
                    {d.highlights.length > 0 && (
                      <div className="mb-4 md:mb-6">
                        {d.highlights.map((h, idx) => (
                          <p key={idx} className="text-white/60 text-xs md:text-sm leading-relaxed mb-2">
                            {h}
                          </p>
                        ))}
                      </div>
                    ...
                    {d.styles.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 md:gap-2">
                        {d.styles.map((s) => (
                          <span
                            key={s}
                            className="px-2 md:px-3 py-0.5 md:py-1 rounded-full bg-white/10 text-white/50 text-[10px] md:text-xs"
                          >
                            {s}
                          </span>
                        ))}
                    ...
                    <div className="mt-6 md:mt-8 text-white/20 text-[10px] md:text-xs">{labels.flipBack}</div>
                  </div>
                </div>
              </motion.div>
            </motion.div>
          ...
          >
            {designs.map((d, i) => {
              ...
              return (
                ...
                >
                  <img src={d.url} alt="" className="w-full h-full object-cover" draggable={false} />
                </motion.div>
              );
            })}
          </motion.div>
            ...
            <div className="relative z-10 w-full h-full flex items-center justify-center overflow-hidden px-4 md:px-16">
              <div className="flex flex-wrap justify-center items-center gap-x-6 md:gap-x-20 gap-y-4 md:gap-y-10 max-w-6xl">
                <div className="group relative">
                  ...
                  >
                    <span
                      className={`text-3xl md:text-7xl lg:text-8xl font-extralight tracking-tighter transition-all duration-500 ${
                  ...
                  <span
                    className={`absolute -top-1 -right-4 md:-top-2 md:-right-6 text-[10px] md:text-xs transition-all duration-500 ${
              ...
              >
                {searchQuery.length > 0 &&
                  (() => {
                    ...
                    const results = allDesigns.filter(
                      (d) =>
                    ...
                    if (results.length === 0) {
                      return <div className="text-center text-white/30 text-lg py-20">{labels.noResults}</div>;
                    }
                ...
                {searchQuery.length === 0 && (
                  <div className="text-center text-white/20 text-sm py-20">{labels.searchEmpty}</div>
                )}
              </div>
            </div>

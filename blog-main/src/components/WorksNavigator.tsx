...
interface WorksNavigatorProps {
  workCount: number;
  workTitles?: string[];
}

export default function WorksNavigator({ workCount, workTitles = [] }: WorksNavigatorProps) {
  const { dictionary } = useI18n();
  const text = dictionary.worksNavigator;
  ...
  useEffect(() => {
    const handleScroll = () => {
      // 获取所有作品的 article 元素
      const articles = document.querySelectorAll('article[data-work-index]');
  ...
  const scrollToWork = (index: number) => {
    const article = document.querySelector(`article[data-work-index="${index}"]`);
    if (article) {
  ...
  const scrollPrev = () => {
    if (activeIndex > 0) {
      scrollToWork(activeIndex - 1);
    }
  };

  const scrollNext = () => {
    if (activeIndex < workCount - 1) {
      scrollToWork(activeIndex + 1);
    }
  };
    ...
    >
          ...
          {Array.from({ length: workCount }).map((_, index) => {
            ...
            return (
              <button
                ...
                aria-label={workTitles[index] || text.goToWork.replace('{index}', String(index + 1))}
              >
      ...
      <div
        ...
        style={{
          ...
          opacity: showTooltip && hoveredIndex !== null && workTitles[hoveredIndex || 0] ? 1 : 0,
        }}
      >

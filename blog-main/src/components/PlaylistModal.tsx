...
const ProgressBar = ({
  ...
  return (
    <div className="relative h-1 group cursor-pointer" onClick={onClick}>
      {/* 背景轨道 */}
      <div className={clsx('absolute inset-0 rounded-full', isDark ? 'bg-white/30' : 'bg-black/10')} />

      {/* 进度条 */}
      <div
        className={clsx(
          'absolute inset-y-0 left-0 rounded-full transition-all',
          isDark ? 'bg-white/90' : 'bg-black/30'
        )}
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] p-0 overflow-hidden rounded-2xl border-0 shadow-2xl">
        ...
        <div className="absolute inset-0 z-0">
          ...
          <Image
            ...
            fill
            className={clsx('object-cover scale-110 blur-2xl', isDark ? 'opacity-20' : 'opacity-10')}
          />
          {/* 渐变遮罩 */}
          <div
            className={clsx(
          ...
          <div
            className={clsx(
              ...
              isDark ? 'from-white/[0.05] to-transparent' : 'from-black/[0.02] to-transparent'
            )}
          />
        </div>
        ...
        <div className="relative z-10 grid grid-cols-[320px_1fr] h-[80vh]">
          ...
          <div className="p-8 flex flex-col">
            ...
            <div className="relative aspect-square w-full rounded-2xl overflow-hidden shadow-[0_15px_40px_-15px_rgba(0,0,0,0.3)]">
              <Image src={currentTrack.cover} alt={currentTrack.name} fill className="object-cover" />
              {/* 封面光效 */}
              <div
                className={clsx(
            ...
            <div className="mt-6 text-center">
              <h2 className={clsx('text-xl font-medium mb-2 tracking-tight', isDark ? 'text-white' : 'text-black')}>
                {currentTrack.name}
              </h2>
              <p className={clsx('text-sm tracking-wide', isDark ? 'text-white/60' : 'text-black/60')}>
                {currentTrack.artist}
              </p>
            </div>
            ...
            <div className="mt-6 flex items-center justify-center gap-8">
              ...
              >
                ...
                />
                {isPlaying ? (
                  <Pause className={clsx('w-6 h-6', isDark ? 'text-white' : 'text-black')} />
                ) : (
                  <Play className={clsx('w-6 h-6 translate-x-0.5', isDark ? 'text-white' : 'text-black')} />
                )}
              </button>
          ...
          <div className="bg-gradient-to-b from-transparent via-black/[0.02] to-black/[0.02] dark:from-transparent dark:via-white/[0.02] dark:to-white/[0.02]">
            <div className="px-8 py-6">
              <h3 className={clsx('text-lg font-medium', isDark ? 'text-white/90' : 'text-black/90')}>播放列表</h3>
              <p className={clsx('text-sm mt-1', isDark ? 'text-white/50' : 'text-black/50')}>
                ...
                {playlist.map((track, index) => (
                  <div
                    ...
                    className={clsx(
                      ...
                      isDark
                        ? ['hover:bg-white/5', currentTrackIndex === index && 'bg-white/[0.08]']
                        : ['hover:bg-black/5', currentTrackIndex === index && 'bg-black/[0.08]']
                    )}
                    onClick={() => {
                      triggerHaptic(HapticFeedback.Heavy);
                      onTrackSelect(index);
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h4
                        className={clsx(
                          ...
                          isDark
                            ? [currentTrackIndex === index ? 'text-white' : 'text-white/70 group-hover:text-white/90']
                            : [currentTrackIndex === index ? 'text-black' : 'text-black/70 group-hover:text-black/90']
                        )}
                      >
                        {track.name}
                      </h4>
                    ...
                    {currentTrackIndex === index && (
                      <div className="flex-shrink-0 w-8 h-8 flex items-center justify-center">
                        {isPlaying ? (
                          <Pause className={clsx('w-5 h-5', isDark ? 'text-white/90' : 'text-black/90')} />
                        ) : (
                          <Play className={clsx('w-5 h-5', isDark ? 'text-white/90' : 'text-black/90')} />
                        )}
                      </div>
                    )}
                  </div>

...
const HEADER_CLASS =
  'fixed top-0 left-0 right-0 z-50 bg-white/40 dark:bg-black/40 backdrop-blur-md border-b border-black/5 dark:border-white/10';
const LINK_CLASS =
  'hover:text-black/60 dark:hover:text-white/60 transition-colors relative after:absolute after:bottom-0 after:left-0 after:right-0 after:h-[1px] after:bg-current after:opacity-0 after:transition-opacity hover:after:opacity-20';
...
export default function Header() {
  ...
  return (
    <>
      ...
      {/* 背景遮罩 - 用于防止内容提前显示 */}
      {isMenuOpen && <div className="fixed inset-0 z-30 bg-white dark:bg-black" aria-hidden="true" />}
    </>
  );
}

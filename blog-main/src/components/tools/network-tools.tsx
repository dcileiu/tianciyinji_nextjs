  ...
  return (
    <div className="space-y-3">
      <Button onClick={() => run()} className={runBtn}>
        {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : t('获取客户端信息')}
      </Button>
...
export function DnsLookupTool() {
  ...
  return (
    <div className="space-y-3">
      <Input className={inputClass} value={dnsHost} onChange={(e) => setDnsHost(e.target.value)} placeholder={t('例如 openai.com')} />
      <Button onClick={() => run({ host: dnsHost })} className={runBtn}>
        {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : t('开始查询')}
      </Button>
      {error && <OutputBox>{error}</OutputBox>}
      {result && (
        <OutputBox className="space-y-2">
  ...
  return (
    <div className="space-y-3">
      <div className="grid gap-3 sm:grid-cols-2">
        <Input className={inputClass} value={pingHost} onChange={(e) => setPingHost(e.target.value)} placeholder={t('目标主机')} />
        <Input className={inputClass} value={pingPort} onChange={(e) => setPingPort(e.target.value)} placeholder={t('端口，例如 443')} />
      </div>
      <Button onClick={() => run({ host: pingHost, port: pingPort, count: 4 })} className={runBtn}>
        {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : t('开始 Ping')}
      </Button>
      {error && <OutputBox>{error}</OutputBox>}
      {result && (
        <OutputBox className="space-y-2">
...
export function PortScanTool() {
  ...
  return (
    <div className="space-y-3">
      <Input className={inputClass} value={portHost} onChange={(e) => setPortHost(e.target.value)} placeholder={t('目标主机')} />
      <Input className={inputClass} value={portExpression} onChange={(e) => setPortExpression(e.target.value)} placeholder={t('例如 80,443,3000-3005')} />
      <Button onClick={() => run({ host: portHost, ports: portExpression })} className={runBtn}>
        {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : t('扫描端口')}
      </Button>
      {error && <OutputBox>{error}</OutputBox>}
      {result && (
        <OutputBox className="space-y-2">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {result.results.map((item: { port: number; open: boolean; latency: number | null }) => (
              <div key={item.port} className="rounded-xl bg-white/60 px-3 py-2 dark:bg-white/[0.03]">
                {item.port} · {item.open ? `${t('开放')} (${item.latency} ms)` : t('关闭')}
              </div>
            ))}
          </div>
        </OutputBox>
...
export function UrlStatusTool() {
  ...
  return (
    <div className="space-y-3">
      <Input className={inputClass} value={urlStatusInput} onChange={(e) => setUrlStatusInput(e.target.value)} placeholder={t('例如 https://openai.com')} />
      <Button onClick={() => run({ url: urlStatusInput })} className={runBtn}>
        {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : t('检测状态')}
      </Button>
      {error && <OutputBox>{error}</OutputBox>}
      {result && (
        <OutputBox className="space-y-2">
...
export function WebMetadataTool() {
  ...
  return (
    <div className="space-y-3">
      <Input className={inputClass} value={metadataUrlInput} onChange={(e) => setMetadataUrlInput(e.target.value)} placeholder={t('输入网页 URL')} />
      <Button onClick={() => run({ url: metadataUrlInput })} className={runBtn}>
        {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : t('提取元数据')}
      </Button>
      {error && <OutputBox>{error}</OutputBox>}
      {result && (
        <OutputBox className="space-y-2">
  ...
  return (
    <div className="space-y-3">
      <Input
        ...
        placeholder={t('输入网页 URL')}
      />
      <Button onClick={() => run({ url: imageExtractUrlInput })} className={runBtn}>
        {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : t('提取图片')}
      </Button>
      {error && <OutputBox>{error}</OutputBox>}
      {result && (
        <OutputBox className="space-y-3">
          <div>{t(`共提取 ${result.images.length} 张图片`)}</div>
...
export function WebMarkdownTool() {
  ...
  return (
    <div className="space-y-3">
      <Input className={inputClass} value={markdownUrlInput} onChange={(e) => setMarkdownUrlInput(e.target.value)} placeholder={t('输入网页 URL')} />
      <Button onClick={() => run({ url: markdownUrlInput })} className={runBtn}>
        {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : t('转换为 Markdown')}
      </Button>
      {error && <OutputBox>{error}</OutputBox>}
      {result && (
        <OutputBox>

...
export default function WeatherCard() {
  ...
  const getWeatherGradient = (text: string) => {
    const gradient =
      Object.entries(weatherGradients).find(([key]) => text.includes(key))?.[1] || weatherGradients['晴'];
  ...
  if (error) return <div className="text-sm text-red-500/80 dark:text-red-400/80">加载天气信息失败</div>;
  ...
  const cardClassName = clsx('rounded-xl overflow-hidden', getWeatherGradient(now.text), 'shadow-lg shadow-black/10');
        ...
        <div className={cardClassName}>
          <div className="p-4">
            ...
            <div className="flex flex-col items-center mb-4">
              <div className="text-6xl font-light text-white mb-2 ml-2">{now.temp}°</div>
              <div className="flex items-center space-x-2">
                <span className="text-lg text-white/90">{now.text}</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 pt-3 border-t border-white/20">
              {forecast.map((day: any) => (
                <div key={day.fxDate} className="text-center">
                  <div className="text-xs text-white/80 mb-1">{formatDate(day.fxDate)}</div>
            ...
            {/* 版权声明 */}
            <div className="mt-3 text-[10px] text-white/60 text-center">数据来源于和风天气</div>
          </div>
        </div>
      </motion.div>
    </AnimatePresence>

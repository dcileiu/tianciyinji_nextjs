...
date: 2026-05-16
excerpt: 从指定源站获取新闻、联网判断最热热点，查库去重后经 GPT 多语言改写与配图自动上线我们自己的CMS；同一套流程也可用于电商跟当天爆品。
author: Dci
...
这套架构本质是 **「发现热点 → 去重 → AI 加工 → 上架自己的站 → 通知」**，换到电商里，只是把「新闻选题」换成 **「当天爆品 / 热搜 SKU」**，把「文章 CMS」换成 **「商品后台 / 独立站」**。

```text
[定时触发]
    → 整合平台 URL（类目榜、店铺、竞品集合页等）
    → 联网搜索 / 榜单 API：判断「今日爆品」候选
    → 查库：SKU / 商品链接是否已跟过
        ├─ 已跟过 → 剔除 → 换下一个爆品
        └─ 未跟过 → 拉商品详情（标题、主图、参数、评价关键词）
            → GPT：改写标题、卖点、详情页文案（多语言）
            → 生成场景图 / 主图变体（注意版权与平台规则）
            → 写入自家站点商品库 → 刷新商品 sitemap → 钉钉「新品已上架」
```
...
| 新闻工作流            | 电商工作流                                             |
| --------------------- | ------------------------------------------------------ |
| 整合资讯站 URL        | 整合淘宝/抖音/亚马逊等类目、榜单、店铺 URL             |
| 联网搜「最热新闻」    | 搜「今日爆品 / 销量飙升 / 热搜词下的款」               |
| 查库：标题 / 原文 URL | 查库：`platform_sku_id`、货源链接、自家 `product_slug` |
| GPT 改写成文章        | GPT 写商品标题、五点描述、详情、SEO 关键词             |
| 生成封面图            | 主图、场景图、对比图（建议自有拍摄或授权图库）         |
| 发布到 CMS 文章       | 发布到 Shopify / 店小秘 / 自研商城 API                 |
| 刷新 sitemap          | 刷新商品页、集合页 sitemap                             |
| 钉钉通知发稿          | 钉钉通知运营：链接、毛利、库存状态                     |

**电商要特别注意的几点：**

- **合规**：平台 ToS、图片版权、是否允许跟卖/搬运详情，比新闻更敏感；爆品信息建议只作选题参考，上架内容尽量 **二次创作 + 自有供应链**。
- **主键**：查库务必用 **平台 SKU + 店铺 ID**（或你自己的货源编码），别只用标题，否则同款不同店会重复上架。
- **GPT 输出**：除了文案，还可以让模型归纳 **差评高频词 → 详情页里主动回应**，或生成 **多语言卖点** 给跨境站。
- **和 ERP 联动**：发布成功后回写库存、成本价、供应商 ID；失败别入库，和新闻「发布成功再存库」同一逻辑。
- **人工闸门**：毛利低于阈值、禁售类目、品牌词命中时，`needs_review` 走待审核，不自动上架。

如果你已经在跑新闻自动化，电商版不必重做一套 n8n：把 `01-ingest` 的 Prompt 和 URL 列表换成电商源，把 `04-publish` 接到商品 API，**查库回环 + 钉钉告警** 几乎可以原样复用。独立站想跟上海外爆品节奏时，这套会比人工刷榜单再抄详情页省太多时间。
...
我们从「半天发一篇多语言稿」压到「热点出现后十几分钟内自动上线」，人工主要维护 URL 白名单、数据库和 Prompt。同一套骨架用在电商上，就是 **跟爆品、查重、AI 商品页、自动上架**——区别主要在数据源和合规，不在工作流形状。

若你也在用 Next.js + Markdown 或自研 CMS / 商城，把发布与 sitemap 两节换成自己的接口即可，**搜热点 + 查库** 这两段可以直接照搬。

...
中间又换过几套方案，CMS、自建后台、各种「博客 + 后台」模板都试过——**第三版**就是这一阶段的产物。每次换完头两周很爽，过两个月又回到老问题：依赖多了、迁移难了、本地起一套环境要半天。
...
| 第三版 | CMS / 模板 / 自建后台 | 短期内换皮换结构很过瘾 | 依赖和迁移成本越来越高 |
| 第四版 | Next.js + 本地 Markdown | 内容和代码在一个仓库，构建托管省心 | 互动能力得按需再加，不能一上来就堆满 |

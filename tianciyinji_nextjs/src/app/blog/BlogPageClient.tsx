...
import { useArticleStore, type Article, type Category, type Pagination } from "@/store/articleStore";
...
export default function BlogPageClient({
  ...
  useLayoutEffect(() => {
    if (!hasServerData || hydrated.current) return;
  ...
  }, [hasServerData, fetchPublishedArticles, fetchCategories]);
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "var(--background)" }}>
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-24">
        <Breadcrumb items={[{ label: "技术博客" }]} className="mb-6" />
        <motion.div
          className="mb-8"
        ...
        >
          {categoriesLoading ? (
            <div className="flex items-center gap-2 mb-4">
              <Loading visible text="加载分类中..." size="small" />
            </div>
          ) : (
              ...
              {allCategories.map((category) => (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor:
                      selectedCategory === category.key ? undefined : "transparent",
          ...
          {displayedArticles.map((article, index) => (
            <Link href={`/blog/${article.id}`} key={article.id}>
              ...
              >
                <div className="relative h-56 overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4">
                    <span className="bg-gradient-to-r from-blue-500 to-purple-600 text-white px-3 py-1.5 rounded-full text-xs font-semibold shadow-lg backdrop-blur-sm">
                      {categories.find((cat) => cat.key === article.category)?.name ||
                        article.category}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-6">
                  <h2
                    className="text-xl font-bold mb-3 line-clamp-2 transition-colors duration-300 group-hover:text-blue-600"
                        ...
                        <span className="relative z-10 flex items-center gap-1">
                          <span
                            ...
                            style={{
                              ...
                              hsl(${(tagIndex * 137 + 180) % 360}, 65%, 60%) 0%, 
                              hsl(${(tagIndex * 137 + 240) % 360}, 70%, 70%) 100%)`,
                            }}
                          />
                          {tag}
                        </span>

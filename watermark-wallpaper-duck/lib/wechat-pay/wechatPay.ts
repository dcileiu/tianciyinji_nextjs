...
export function getPayConfig() {
  ...
  if (!appid || !mchid || !serialNo || !privateKey || !notifyUrl) {
    throw new Error(
      "微信支付配置不完整，请检查 WECHAT_APPID / WECHAT_MCH_ID / WECHAT_PAY_SERIAL_NO / WECHAT_PAY_PRIVATE_KEY / WECHAT_PAY_NOTIFY_URL",
    );
  }
...
function signWithMerchantKey(message: string, privateKey: string) {
